...
export default function IntonationTab() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="premium-card p-6 bg-gradient-to-r from-indigo-50 to-purple-50 border-indigo-200">
        <h3 className="text-lg font-black text-indigo-900 flex items-center gap-2 mb-2">
          <MessageSquare className="w-5 h-5 text-indigo-600" /> Ngữ Điệu Câu & Trọng Âm Câu (Intonation & Sentence Stress)
        </h3>
      ...
      {intonationSubTab === 'questions' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {QUESTION_TYPES_DATA.map((q, idx) => (
            <div key={idx} className="p-6 bg-white border border-slate-200 rounded-3xl space-y-4 shadow-xs">
              <div className="flex items-center justify-between">
                <h4 className="font-extrabold text-sm text-indigo-900">{q.title || q.type}</h4>
                <span className="text-[10px] font-black uppercase tracking-wider px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-full border border-indigo-100">
                  {(q as any).pattern || q.intonation}
                </span>
              </div>
              <p className="text-xs text-slate-600 leading-relaxed font-medium">{(q as any).description || q.desc}</p>
              {q.examples && (
                <div className="space-y-2 pt-2 border-t border-slate-100">
                  {q.examples.map((ex: any, i: number) => (
                    <div key={i} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-xl text-xs font-semibold">
                      <span className="text-slate-800">{ex.sentence || ex.text}</span>
                      <button
                        onClick={() => speak(ex.sentence || ex.text)}
                        className="p-1.5 bg-indigo-100 text-indigo-700 rounded-lg hover:bg-indigo-600 hover:text-white transition-all cursor-pointer"
                      >
      ...
      {intonationSubTab === 'intonation' && (
        <div className="space-y-4">
          {INTONATION_GUIDELINES.map((g: any, idx: number) => (
            <div key={idx} className="p-6 bg-white border border-slate-200 rounded-3xl space-y-3">
              <h4 className="font-extrabold text-sm text-slate-800">{g.pattern || g.rule}</h4>
              <p className="text-xs text-slate-600 font-medium">{g.desc || g.detail}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {intonationSubTab === 'sentence_stress' && (
        <div className="space-y-4">
          {SENTENCE_STRESS_RULES.map((r: any, idx: number) => (
            <div key={idx} className="p-6 bg-white border border-slate-200 rounded-3xl space-y-3">
              <h4 className="font-extrabold text-sm text-slate-800">{r.category || r.title}</h4>
              <p className="text-xs text-slate-600 font-medium">{r.desc || r.content}</p>
            </div>
          ))}
        </div>
      )}
