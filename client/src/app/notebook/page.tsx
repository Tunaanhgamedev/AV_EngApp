...
const MASTERY_CLR = ['bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300', 'bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400', 'bg-yellow-100 dark:bg-yellow-500/10 text-yellow-700 dark:text-yellow-400', 'bg-orange-100 dark:bg-orange-500/10 text-orange-700 dark:text-orange-400', 'bg-green-100 dark:bg-green-500/10 text-green-700 dark:text-green-400', 'bg-purple-100 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400'];
const TYPE_COLORS: Record<string, string> = { noun: 'bg-sky-100 dark:bg-sky-500/10 text-sky-700 dark:text-sky-400', verb: 'bg-rose-100 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400', adjective: 'bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-400', adverb: 'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400', phrase: 'bg-teal-100 dark:bg-teal-500/10 text-teal-700 dark:text-teal-400', idiom: 'bg-pink-100 dark:bg-pink-500/10 text-pink-700 dark:text-pink-400' };
...
function WordCard({ word, onDelete, onUpdate }: { word: NotebookWord; onDelete: (id: string) => void; onUpdate: (id: string, fields: any) => Promise<void> }) {
  ...
  const typeColor = TYPE_COLORS[word.wordType?.toLowerCase()] || 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400';

  if (isEditing) {
    return (
      <div className="premium-card p-6 border-2 border-primary/20 bg-gradient-to-br from-white dark:from-slate-900 to-primary/5 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 mb-4">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
            <h3 className="text-sm font-black text-slate-800 dark:text-slate-200 uppercase tracking-wider">Chỉnh sửa: <span className="text-primary font-black">{word.word}</span></h3>
          </div>
          <button 
            type="button" 
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 block">Phiên âm</label>
              <input 
                type="text" 
                value={editPhonetic} 
                onChange={e => setEditPhonetic(e.target.value)} 
                placeholder="/.../" 
                className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-primary/20 focus:outline-none"
              />
            </div>

            <div>
              <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 block">Loại từ</label>
              <select 
                value={editWordType} 
          ...
          <div>
            <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 block">Định nghĩa tiếng Anh</label>
            <textarea 
              value={editMeaningEn} 
              onChange={e => setEditMeaningEn(e.target.value)} 
              rows={2}
              placeholder="English definition" 
              className="w-full p-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-medium focus:ring-2 focus:ring-primary/20 focus:outline-none resize-none"
            />
          </div>

          <div>
            <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 block">Nghĩa tiếng Việt</label>
            <textarea 
              value={editMeaningVi} 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 block">Ví dụ tiếng Anh</label>
              <textarea 
                value={editExample} 
            ...
            <div>
              <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 block">Dịch ví dụ tiếng Việt</label>
              <textarea 
                value={editExampleVi} 
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
            <button 
              type="button" 
              onClick={() => setIsEditing(false)} 
              className="px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg font-black text-[10px] uppercase tracking-wider transition-colors"
            >
              HỦY
            </button>
            <button 
              type="submit" 
              disabled={submitting}
              className="px-4 py-2 bg-slate-900 dark:bg-slate-100 hover:bg-slate-800 dark:hover:bg-slate-200 text-white dark:text-slate-900 rounded-lg font-black text-[10px] uppercase tracking-wider transition-colors flex items-center gap-1.5 shadow-md shadow-slate-900/10"
            >
              {submitting ? (
                <>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-xl font-black text-slate-900 dark:text-slate-100">{word.word}</h3>
              {word.phonetic && <span className="text-sm text-slate-400 dark:text-slate-500 font-mono">{word.phonetic}</span>}
              <button onClick={e => { e.stopPropagation(); speak(word.word); }} className="p-1 text-slate-300 dark:text-slate-600 hover:text-primary transition-colors"><Volume2 className="w-4 h-4" /></button>
            </div>
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              ...
              {word.cefrLevel && <span className="text-[10px] font-bold px-2 py-0.5 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-full">{word.cefrLevel}</span>}
          ...
          <div className="flex items-center gap-1">
            <button 
              ...
              className="p-1.5 text-slate-200 dark:text-slate-700 hover:text-primary transition-all"
              title="Chỉnh sửa từ vựng"
            >
              <Pencil className="w-4 h-4" />
            </button>
            <button onClick={e => { e.stopPropagation(); onDelete(word.id); }} className="p-1.5 text-slate-200 dark:text-slate-700 hover:text-rose-500 transition-all"><Trash2 className="w-4 h-4" /></button>
            <ChevronDown className={cn("w-5 h-5 text-slate-300 dark:text-slate-600 transition-transform", expanded && "rotate-180")} />
          </div>
        </div>
        {!expanded && (
          <div className="mt-3 space-y-0.5">
            <p className="text-sm text-slate-600 dark:text-slate-300 font-medium line-clamp-1">{word.meaningEn}</p>
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 animate-in slide-in-from-top-2 duration-300">
          <div className="flex border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
            {(['definition', 'usage', 'pronunciation'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} className={cn("flex-1 py-3 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1.5", tab === t ? "text-primary border-b-2 border-primary bg-white dark:bg-slate-900" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300")}>
                  ...
                  <div className="space-y-4">
                    {insight ? insight.wordTypes.map((wt, i) => (
                      <div key={i} className="flex gap-3">
                        <span className={cn("text-[10px] font-black px-2 py-0.5 rounded-full uppercase h-fit mt-0.5 flex-shrink-0", TYPE_COLORS[wt.type?.toLowerCase()] || 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400')}>{wt.type}</span>
                        <div>
                          <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 leading-relaxed">{wt.meaningEn}</p>
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-3">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 leading-relaxed">{word.meaningEn || 'Definition not found'}</p>
                            <p className="text-sm text-primary italic mt-0.5">{word.meaningVi}</p>
                          </div>
                        </div>
                        {word.example && (
                          <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-800">
                            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 block">Ví dụ:</p>
                            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">"{word.example}"</p>
                            {word.exampleVi && <p className="text-[11px] text-slate-400 dark:text-slate-500 italic mt-0.5">{word.exampleVi}</p>}
                          </div>
                        )}
                        <div className="p-4 bg-amber-50 dark:bg-amber-500/10 rounded-2xl border border-amber-100 dark:border-amber-500/20 flex items-center gap-3">
                          <AlertCircle className="w-5 h-5 text-amber-500" />
                          <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">AI hiện đang bận hoặc từ này không có trong từ điển. Thử nhấn nút làm mới bên dưới.</p>
                        </div>
                    ...
                    {insight?.tip && (
                      <div className="flex gap-2 p-3 bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-2xl mt-3">
                        <Lightbulb className="w-4 h-4 text-blue-500 flex-shrink-0" />
                        <p className="text-sm text-blue-700 dark:text-blue-400 font-medium">{insight.tip}</p>
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    {insight?.usageExamples?.map((ex, i) => (
                      <div key={i} className="rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
                        <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800 flex items-center gap-2"><span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{ex.context}</span></div>
                        <div className="p-4"><p className="text-sm font-semibold text-slate-800 dark:text-slate-200">"{ex.sentence}"</p><p className="text-xs text-slate-400 dark:text-slate-500 italic mt-1">{ex.sentenceVi}</p></div>
                      </div>
                    )) || (word.example ? (
                      <div className="rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
                        <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800 flex items-center gap-2"><span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Ví dụ</span></div>
                        <div className="p-4"><p className="text-sm font-semibold text-slate-800 dark:text-slate-200">"{word.example}"</p>{word.exampleVi && <p className="text-xs text-slate-400 dark:text-slate-500 italic mt-1">{word.exampleVi}</p>}</div>
                      </div>
                    ) : (
                      <div className="text-center py-8 text-slate-300 dark:text-slate-600">Không có ví dụ cho từ này</div>
                    ))}
                  </div>
                )}
                {tab === 'pronunciation' && (
                  <div className="flex flex-col items-center gap-5 py-4">
                    <div className="text-5xl font-black text-slate-800 dark:text-slate-200">{word.word}</div>
                    <span className="text-2xl font-mono text-slate-400 dark:text-slate-500">{insight?.phonetic || word.phonetic || '—'}</span>
...
function AddWordPanel({ onAdd }: { onAdd: (d: any) => Promise<void> }) {
  ...
  return (
    <div className="premium-card p-6 border-2 border-dashed border-primary/30 bg-gradient-to-br from-primary/5 to-purple-50 dark:to-purple-950/30">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className={cn("w-full pl-10 pr-4 py-3.5 bg-white dark:bg-slate-900 border border-primary/20 rounded-xl text-base font-medium transition-all focus:outline-none focus:ring-2", isTooLong && "border-rose-300 dark:border-rose-500/50 ring-rose-100 dark:ring-rose-500/20")}
            />
          </div>
        </div>
        ...
        {expanded && word.trim() && (
          <div className="space-y-4 animate-in slide-in-from-top-2">
            {isTooLong ? (
              <div className="p-4 bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 rounded-2xl flex items-start gap-3 border border-rose-100 dark:border-rose-500/20">
              ...
              <>
                <div>
                  <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 block">Loại từ</label>
                  <div className="flex flex-wrap gap-2">
                    {['noun', 'verb', 'adjective', 'adverb', 'other'].map(t => (
                      <button key={t} type="button" onClick={() => setWordType(t)} className={cn("px-4 py-2 rounded-xl text-xs font-bold border-2 transition-all", wordType === t ? "bg-primary text-white border-primary" : "bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700")}>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 block">Nghĩa tiếng Việt</label>
                  <textarea value={meaningVi} onChange={e => setMeaningVi(e.target.value)} placeholder="Nghĩa của từ này là gì?" rows={2} className="w-full p-3 border-2 border-slate-100 dark:border-slate-800 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:outline-none resize-none" />
                </div>
                <div className="flex items-center gap-3">
                  <button type="submit" disabled={loading} className="flex-1 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 py-3 rounded-xl font-bold text-sm shadow-xl hover:scale-[1.02] active:scale-95 transition-all">
                    {loading ? 'Đang lưu...' : 'Lưu vào Notebook'}
                  </button>
                  <button type="button" onClick={reset} className="px-6 py-3 text-slate-400 dark:text-slate-500 font-bold text-sm">Hủy</button>
                </div>
              </>
            )}
          </div>
...
export default function NotebookPage() {
  ...
  if (!user) return (
    <div className="flex flex-col items-center justify-center h-[60vh] text-center p-6">
      ...
      <Link 
        href="/login" 
        className="mt-6 px-8 py-3 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl font-bold shadow-xl flex items-center justify-center gap-2 hover:bg-slate-800 dark:hover:bg-slate-200 transition-all cursor-pointer"
      >
  ...
  return (
    <div className="space-y-8 pb-10">
      {toast && <div className={cn("fixed top-6 right-6 z-50 px-6 py-3 rounded-xl font-bold text-sm shadow-2xl animate-in slide-in-from-right-4", toast.ok ? "bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900" : "bg-rose-500 text-white")}>{toast.msg}</div>}
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">Lưu trữ từ vựng đơn lẻ để ôn tập chuyên sâu cùng EngBot AI.</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push('/review')}
            className="flex items-center gap-2 px-4 py-2 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl font-bold hover:scale-105 transition-all shadow-lg"
          >
      ...
      <div className={cn(
        ...
        bannerState === 'idle' && "bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 border-2 border-slate-100 dark:border-slate-800"
      )}>
        ...
        <div className="space-y-1 relative z-10 text-center sm:text-left">
          ...
          <p className={cn("font-medium", bannerState === 'idle' ? "text-slate-500 dark:text-slate-400" : "text-white/80")}>
        ...
        <button
          ...
          className={cn(
            ...
            bannerState === 'idle' && "bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900",
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Tìm kiếm trong Notebook..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/10 placeholder:text-slate-400 dark:placeholder:text-slate-500" />
      </div>
