...
const MOCK_LEADERBOARD = [
  { id: 1, name: "Alex Rivera", xp: 12450, avatar: "A", trend: "up", level: 42, color: "bg-amber-100 text-amber-600" },
  { id: 2, name: "Sarah Chen", xp: 11200, avatar: "S", trend: "down", level: 38, color: "bg-slate-100 text-slate-600" },
  { id: 3, name: "Moi Tuấn", xp: 9800, avatar: "M", trend: "up", level: 35, color: "bg-orange-100 text-orange-600", isMe: true },
  { id: 4, name: "Elena Gilbert", xp: 8500, avatar: "E", trend: "same", level: 31, color: "bg-blue-100 text-blue-600" },
  { id: 5, name: "David Kim", xp: 7200, avatar: "D", trend: "up", level: 28, color: "bg-purple-100 text-purple-600" },
  { id: 6, name: "Sophia Miller", xp: 6800, avatar: "S", trend: "down", level: 25, color: "bg-pink-100 text-pink-600" },
  { id: 7, name: "James Wilson", xp: 5900, avatar: "J", trend: "up", level: 22, color: "bg-green-100 text-green-600" },
];
    ...
    <div className="max-w-4xl mx-auto space-y-10 py-6 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          <div className="px-4 py-2 bg-rose-50 text-rose-600 rounded-xl flex items-center gap-2 border border-rose-100">
            <Timer className="w-4 h-4" />
            <span className="text-sm font-black uppercase tracking-widest">2d 14h left</span>
          </div>
        </div>
      </header>
      ...
      <section className="grid grid-cols-3 gap-4 md:gap-8 items-end pt-10 pb-6 px-4">
        ...
        <div className="flex flex-col items-center space-y-4">
          <div className="relative">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-slate-200 flex items-center justify-center text-2xl font-black text-slate-500 border-4 border-white shadow-xl">
        ...
        <div className="flex flex-col items-center space-y-4 scale-110 md:scale-125 z-10">
          <div className="relative">
            ...
            <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-yellow-100 flex items-center justify-center text-3xl font-black text-yellow-600 border-4 border-yellow-400 shadow-2xl shadow-yellow-200">
        ...
        <div className="flex flex-col items-center space-y-4">
          <div className="relative">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-orange-100 flex items-center justify-center text-2xl font-black text-orange-600 border-4 border-white shadow-xl">
              {MOCK_LEADERBOARD[2].avatar}
            </div>
            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-orange-500 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold border-2 border-white">3</div>
          </div>
          <div className="text-center">
        ...
        <div className="divide-y divide-slate-50">
          {MOCK_LEADERBOARD.map((player, i) => (
            <div 
              key={player.id} 
              className={cn(
                "p-4 md:p-6 flex items-center gap-4 transition-all hover:bg-slate-50 group",
                player.isMe ? "bg-primary/5 ring-1 ring-inset ring-primary/10" : ""
              )}
            >
              <div className="w-8 text-center font-black text-slate-300 group-hover:text-primary transition-colors">
                {i + 1}
              </div>
              ...
              <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg shadow-sm border border-white", player.color)}>
                {player.avatar}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Level {player.level}</p>
              </div>

              <div className="text-right">
                <p className="text-lg font-black text-slate-800">{player.xp.toLocaleString()}</p>
                <p className="text-[10px] font-black text-primary uppercase tracking-widest">Total XP</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="p-6 bg-slate-900 text-white relative overflow-hidden">
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="font-bold">Climbing Fast!</h4>
                <p className="text-xs text-slate-400">You earned <span className="text-white font-bold">1,200 XP</span> this week.</p>
              </div>
            </div>
