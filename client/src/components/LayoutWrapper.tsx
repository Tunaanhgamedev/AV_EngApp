...
export function LayoutWrapper({ children }: LayoutWrapperProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col w-full relative">
      {/* Mobile Topbar */}
      <header className="lg:hidden h-16 border-b border-slate-200/50 dark:border-slate-700/50 bg-background/80 backdrop-blur-md fixed top-0 left-0 right-0 z-40 px-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[8px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">AI English Mentor</p>
          </div>
        </div>
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all active:scale-95 shadow-sm"
