...
${wordList}
   ...
   */
  static async generateIeltsStudyPlan(stats: { listening: number; reading: number; writing: number; speaking: number; overallAvg: number }) {
    try {
      const prompt = `
   ...
   */
  static async evaluateToeicWriting(challenge: any, userText: string) {
    try {
      const prompt = `
   ...
   */
  static async generateReadingArticle(level: string) {
    try {
   ...
   */
  static async generateListeningLesson(level: string) {
    try {
      const targetLevel = level || 'B1';
      const prompt = `
