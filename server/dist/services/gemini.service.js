...
class GeminiService {
    ...
    static quizCache = new Map();
    static CACHE_TTL_MS = 15 * 60 * 1000; // 15 minutes TTL
    static getFromQuizCache(key) {
    ...
    static setToQuizCache(key, data) {
     ...
     */
    static getSkillsIndex() {
        if (GeminiService.cachedSkills) {
            return GeminiService.cachedSkills;
        }
        try {
            const possibleIndexPaths = [
                path_1.default.join(process.cwd(), '../antigravity-awesome-skills/skills_index.json'),
                path_1.default.join(__dirname, '../../../antigravity-awesome-skills/skills_index.json'),
                path_1.default.join(__dirname, '../../../../antigravity-awesome-skills/skills_index.json'),
                path_1.default.join(process.cwd(), 'antigravity-awesome-skills/skills_index.json'),
            ];
            let indexPath = '';
            ...
            if (indexPath) {
                ...
                if (Array.isArray(parsed)) {
                    ...
                    GeminiService.cachedSkills = parsed.map((item) => ({
     ...
     */
    static async retrieveSpecificSkillsContext(skills) {
        try {
            const possiblePaths = [
                path_1.default.join(process.cwd(), '../antigravity-awesome-skills/skills'),
                path_1.default.join(__dirname, '../../../antigravity-awesome-skills/skills'),
                path_1.default.join(__dirname, '../../../../antigravity-awesome-skills/skills'),
                path_1.default.join(process.cwd(), 'antigravity-awesome-skills/skills'),
            ];
            let skillsDir = '';
            ...
            for (const skillId of skills) {
                const safeSkillId = path_1.default.basename(skillId);
                const dirPath = path_1.default.join(skillsDir, safeSkillId);
                if (fs_1.default.existsSync(dirPath) && fs_1.default.statSync(dirPath).isDirectory()) {
                    ...
                    if (mdFile) {
                        const filePath = path_1.default.join(dirPath, mdFile);
     ...
     */
    static async retrieveSkillContext(message) {
        try {
            const possiblePaths = [
                path_1.default.join(process.cwd(), '../antigravity-awesome-skills/skills'),
                path_1.default.join(__dirname, '../../../antigravity-awesome-skills/skills'),
            ...
            for (const skill of skills) {
                ...
                for (const word of queryWords) {
                    ...
                    if (skill.tags.some((t) => t.toLowerCase() === word)) {
                        score += 6;
                    }
                    ...
                    if (normalizedMessage.includes(dirLower)) {
                        ...
                        for (const part of parts) {
                            if (part.length <= 3) {
                                ...
                                if (normalizedMessage.includes(part))
                                    matchCount++;
                            }
                        }
            ...
            for (const dir of uniqueMatched) {
                const dirPath = path_1.default.join(skillsDir, dir);
                if (fs_1.default.existsSync(dirPath) && fs_1.default.statSync(dirPath).isDirectory()) {
                    const files = fs_1.default.readdirSync(dirPath);
                    const mdFile = files.find(f => f.toLowerCase() === 'skill.md' || f.toLowerCase() === 'readme.md' || f.endsWith('.md'));
                    if (mdFile) {
...
`;
            ...
            if (yesterdayPresentMatch) {
                ...
                const verbMap = { "i go": "I went", "i eat": "I ate", "i see": "I saw", "i buy": "I bought", "i come": "I came", "i have": "I had", "i take": "I took", "i make": "I made", "i do": "I did", "i get": "I got", "i run": "I ran", "i give": "I gave", "i think": "I thought", "i say": "I said", "i tell": "I told" };
...
${wordList}
     ...
     */
    static async generateToeicFullTest(mode = 'standard') {
        const cacheKey = `toeic_full_test_${mode}`;
        const cached = GeminiService.getFromQuizCache(cacheKey);
        if (cached)
            return cached;
        const parts = mode === 'mini' ? [1, 3, 5, 7] : [1, 2, 3, 4, 5, 6, 7];
        const results = await Promise.all(parts.map(part => GeminiService.generateToeicPractice(part, mode)));
     ...
     */
    static async generateIeltsFullTest(mode = 'standard') {
        const cacheKey = `ielts_full_test_${mode}`;
        const cached = GeminiService.getFromQuizCache(cacheKey);
        if (cached)
            return cached;
        const skills = ['listening', 'reading', 'writing', 'speaking'];
        const results = await Promise.all(skills.map(skill => GeminiService.generateIeltsPractice(skill, mode)));
     ...
     */
    static async logUnresolvedQuestion(userId, questionText, errorContext) {
        try {
            const uId = userId || null;
            await prisma_1.default.unresolvedQuestion.create({
                data: {
                    userId: uId,
                    questionText,
            ...
            try {
                const filePath = path_1.default.join(__dirname, '../../unresolved_questions.json');
                let questions = [];
                ...
                fs_1.default.writeFileSync(filePath, JSON.stringify(questions, null, 2), 'utf8');
     ...
     */
    static async verifyGrammarTense(tenseName, userSentence) {
        try {
            const prompt = `
     ...
     */
    static async generateTenseQuiz(tenses) {
        try {
