...
router.post('/analyze-stress', async (req, res) => {
    ...
    try {
        ...
        if (!bypassCache) {
            const cachedAnalysis = await prisma_1.default.wordStressAnalysis.findUnique({
      ...
      `;
        ...
        try {
            await prisma_1.default.wordStressAnalysis.upsert({
                where: { word: cleanWord },
                update: {
...
router.post('/verify-grammar-tense', async (req, res) => {
    ...
    try {
        const result = await gemini_service_1.GeminiService.verifyGrammarTense(tenseName, userSentence);
...
router.post('/generate-tense-quiz', async (req, res) => {
    ...
    try {
        const quiz = await gemini_service_1.GeminiService.generateTenseQuiz(tenses);
